...
test('MCP server: initialize, list tools, save/list/get/delete, status, isolation', async () => {
  ...
  try {
    ...
    const s1 = await mcp.toolCall('memory_save', {
      cwd: cwdA,
      type: 'semantic',
      title: 'tabs vs spaces',
      content: 'we use tabs, no semicolons',
      tags: ['style', 'code'],
    });
    ...
    const s2 = await mcp.toolCall('memory_save', {
      cwd: cwdA,
      type: 'procedural',
      title: 'release',
      content: 'git tag && git push --tags',
      tags: ['ci'],
    });
    ...
    const sB = await mcp.toolCall('memory_save', {
      cwd: cwdB,
      type: 'semantic',
      content: 'beta only',
      tags: [],
    ...
    const listA = JSON.parse(
      (await mcp.toolCall('memory_list', { cwd: cwdA, scope: 'project', limit: 10 })).content[0]
        .text,
    );
    const listB = JSON.parse(
      (await mcp.toolCall('memory_list', { cwd: cwdB, scope: 'project', limit: 10 })).content[0]
        .text,
    );
    ...
    const recall = JSON.parse(
      (await mcp.toolCall('memory_recall', { cwd: cwdA, scope: 'project', query: 'tabs' }))
        .content[0].text,
    );
    ...
    for (const item of recall.items)
      assert.equal(
        item.scope || 'project',
        'project',
        'recall items should not carry a global tag for scope=project',
      );

    // Update the title via scope=project.
    const upd = JSON.parse(
      (
        await mcp.toolCall('memory_update', {
          cwd: cwdA,
          scope: 'project',
          id: j1.memory.id,
          title: 'tabs vs spaces (updated)',
        })
      ).content[0].text,
    );
    ...
    const sup = JSON.parse(
      (
        await mcp.toolCall('memory_save', {
          ...
          type: 'semantic',
          title: 'tabs vs spaces',
          content: 'we use 2 spaces, no semicolons',
          supersede: true,
        })
      ).content[0].text,
    );
    ...
    const wmg = JSON.parse(
      (await mcp.toolCall('working_memory_get', { cwd: cwdA, slot: 'current_focus' })).content[0]
        .text,
    );
    assert.equal(wmg.value, 'ship the demo');
    const wmc = JSON.parse(
      (await mcp.toolCall('working_memory_clear', { cwd: cwdA, slot: 'current_focus' })).content[0]
        .text,
    );
    ...
    const del = JSON.parse(
      (await mcp.toolCall('memory_delete', { cwd: cwdA, scope: 'project', id: j2.memory.id }))
        .content[0].text,
    );
    assert.equal(del.operation, 'deleted');
    assert.equal(del.scope, 'project');
    assert.equal(del.deleted, true);
    const getDel = await mcp.toolCall('memory_get', {
      cwd: cwdA,
      scope: 'project',
      id: j2.memory.id,
    });
    ...
    const badCwd = await mcp.toolCall('memory_save', {
      cwd: 'relative/path',
      type: 'semantic',
      content: 'x',
    });
    assert.equal(badCwd.isError, true);
    const badScope = await mcp.toolCall('memory_save', {
      cwd: cwdA,
      scope: 'all',
      type: 'semantic',
      content: 'x',
    ...
    const badReadScope = await mcp.toolCall('memory_recall', {
      cwd: cwdA,
      scope: 'lol',
      query: 'tabs',
    });
...
test('MCP server: global memory saved from project A is recalled from project B and project memories never cross projects', async () => {
  ...
  try {
    ...
    const gSave = await mcp.toolCall('memory_save', {
      cwd: cwdA,
      scope: 'global',
      type: 'semantic',
      title: 'theme preference',
      content: 'User prefers dark themes everywhere',
      tags: ['theme'],
    });
    ...
    const projSave = await mcp.toolCall('memory_save', {
      cwd: cwdA,
      type: 'semantic',
      title: 'proj only doc',
      content: 'this is private project A info',
      tags: [],
    });
    ...
    const recallB = JSON.parse(
      (await mcp.toolCall('memory_recall', { cwd: cwdB, query: 'theme preference' })).content[0]
        .text,
    );
    ...
    for (const item of recallB.items)
      assert.equal(item.scope, 'global', 'all-scope results carry a scope tag');

    // explicit scope="global" + cwdB finds the same global row.
    const globList = JSON.parse(
    ...
    const got = JSON.parse(
      (await mcp.toolCall('memory_get', { cwd: cwdB, scope: 'global', id: gId })).content[0].text,
    );
    ...
    const miss = await mcp.toolCall('memory_update', {
      cwd: cwdB,
      scope: 'project',
      id: gId,
      title: 'should fail',
...
test('MCP server: scope="all" merge tags items by scope and preserves project-first order', async () => {
  ...
  try {
    ...
    await mcp.toolCall('memory_save', {
      ...
      scope: 'global',
      type: 'semantic',
      title: 'global codestyle',
      content: 'globally we prefer tabs',
      tags: ['style'],
    });

    const merged = JSON.parse(
      (await mcp.toolCall('memory_recall', { cwd, scope: 'all', query: 'tabs' })).content[0].text,
    );
...
test('MCP server: memory_save_bulk saves many rows in one transaction and supersedes within the batch', async () => {
  ...
  try {
    ...
    const list = JSON.parse(
      (await mcp.toolCall('memory_list', { cwd, scope: 'project', limit: 50 })).content[0].text,
    );
    assert.equal(list.count, 3);
    // Bad input: bad type is rejected.
    const bad = await mcp.toolCall('memory_save_bulk', {
      cwd,
      items: [{ type: 'banana', content: 'x' }],
    });
    assert.equal(bad.isError, true);
