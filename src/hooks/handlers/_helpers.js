// Shared helpers for the per-event hook handlers. Pure (no side effects
// on import); consumed by every file under src/hooks/handlers/.
//
// Constants, payload adapters, count builders, segment formatters, and
// the safe ingest path all live here so a per-event handler can stay
// short and event-specific. The dispatcher (src/hooks/run.js) wires
// HOME / EVENT from the runtime; handlers import them from there.

import path from 'node:path';
import { existsSync } from 'node:fs';
import { kimiHome, nowIso, asString, readStdin, safeJsonParse, PATH_REGEX } from '../../util.js';
import {
  canonicalizeRoot,
  deriveProjectKey,
  ensureProjectDir,
...
// Re-export the session-focus helpers so per-event handlers can pull
// them from one place. They are also re-exported from session-focus.js
// directly; this is the canonical entry point for handlers.
...
// HOME is set at module load by the dispatcher so every handler sees
// the same value. EVENT is also dispatcher-supplied.
...
// ---- Status-line segment formatters ----
