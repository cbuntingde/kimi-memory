...
import { kimiHome, nowIso, sanitizeText } from './util.js';
...
// Recursively scrub a context object. Keys are preserved (they are
// author-controlled identifiers); every string value is scrubbed.
function scrubContext(value, depth = 0) {
  ...
  if (value && typeof value === 'object') {
    const out = {};
    for (const k of Object.keys(value)) out[k] = scrubContext(value[k], depth + 1);
    return out;
  }
  return value;
}
...
function scrubError(error) {
  ...
  return scrub((error && error.message) || String(error)) || 'unknown error';
}
...
export async function logHookError(event, hookName, error, context = {}) {
  const record = {
    ...
    // Scrubbed, not raw: a stack embeds absolute file paths and, on a
    // fetch failure, the request URL. Truncated to keep a pathological
    // stack from dominating the log.
    stack: error?.stack ? scrub(error.stack).slice(0, 2000) : null,
...
export async function logAutoExtractError(projectKey, reason, error, context = {}) {
  const record = {
    ...
    context: scrubContext(context.extra || {}),
  };
  await appendLog(record);
}
