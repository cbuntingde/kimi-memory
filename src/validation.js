...
// Edge kind vocabulary. Canonical list lives in ./edge-kinds.js so the
// validator, the graph layer, and the SQL CHECK constraint cannot drift
// apart again — they had, and this validator was rejecting the three
// CodeGraph kinds the schema accepted.
...
export function validateEdgeKind(v) {
  ...
  if (!EDGE_KIND_SET.has(v))
    return { ok: false, error: `kind must be one of: ${EDGE_KIND_LIST.join(', ')}` };
  return { ok: true, value: v };
}
