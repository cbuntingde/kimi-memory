...
export async function cmdImport(args) {
  ...
  if (process.env.KIMI_MEMORY_SECRET_SCAN !== 'off') {
    ...
    // Working-memory slots land verbatim in working_memory.value and are
    // surfaced back into the agent context by the SessionStart preview,
    // so they are as sensitive as a durable row. Scanning only the
    // `memories` arrays left this channel open.
    if (doc.scopes.project?.working_memory) {
      for (const wm of doc.scopes.project.working_memory) {
        all.push({ id: `working_memory:${wm && wm.slot}`, content: wm && wm.value });
      }
    }
    for (const m of all) {
      try {
  ...
  if ((scope === 'project' || scope === 'all') && doc.scopes.project) {
    ...
    for (const wm of doc.scopes.project.working_memory || []) {
      try {
        // Routed through setWorkingMemory so the imported slot passes the
        // same `assertNoSecret` gate the live MCP tool does. The direct
        // INSERT that used to live here bypassed it.
        setWorkingMemory(db, key, wm.slot, wm.value);
      } catch (e) {
        process.stderr.write(
