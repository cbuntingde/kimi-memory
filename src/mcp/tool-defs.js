...
// definitions are present in this array. That behaviour is verified
// by tests/06-manifest.test.js; there is deliberately no separate
// schema-time "active tools" list, because a second copy of the
// gated names would be one more thing to keep in sync with the
// three handler modules that actually enforce the gate.
...
export const TOOL_DEFS = [
  ...
  },
  {
    ...
    input: {
      ...
      // vector channels.
      visibility: z
        .union([z.enum(VISIBILITY_LEVELS), z.array(z.enum(VISIBILITY_LEVELS)).max(5)])
        .optional()
  ...
  },
  {
    ...
    input: {
      ...
      visibility: z.enum(VISIBILITY_LEVELS).optional().describe('v10: row visibility.'),
      shared_with: z
        .array(z.string().min(1).max(128))
        .max(32)
        .optional()
  ...
  },
  {
    ...
    input: {
      ...
      visibility: z.enum(VISIBILITY_LEVELS).describe('New visibility level.'),
      shared_with: z
        .array(z.string().min(1).max(128))
        .max(32)
        .optional()
