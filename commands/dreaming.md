...
The MCP surface is a single tool, `dreaming`, that takes one JSON-string
`args` object. The `operation` field in the response names the sub it
ran — `dreaming_status`, `dreaming_set`, `dreaming_run`, `dreaming_last`.
...
1. Read the current state:
   ...
   ```
   dreaming({ cwd, args: '{"sub":"status"}' })
   ```

   (or `node src/cli.js dreaming status --cwd <path>`). The response
   carries the effective mode, interval, include set, last run
   timestamp, next due time, and which file (project vs global) is the
   source.

2. For state transitions (`on|off|auto`), pass the change in `args`:

   ```
   dreaming({ cwd, args: '{"sub":"on","scope":"project","interval_spec":"3h","include":"consolidate,dream,gc"}' })
   ```

   `scope` defaults to `project`; pass `"global"` to set the system-wide
   default. `interval_spec` accepts values like `30m`, `3h`, `24h`, `1d`;
   `interval_ms` is the integer-millisecond alternative. The
   `KIMI_MEMORY_DREAMING_MODE` and `KIMI_MEMORY_DREAMING_INTERVAL_MS`
   env vars override the file for the current process.

3. Print the resulting state so the user sees what landed.

For `run`:

1. ```
   dreaming({ cwd, args: '{"sub":"run","exclude":"gc"}' })
   ```
   The `force` flag is implicit; the interval floor does not apply. The
   include list, when supplied, replaces the configured include set for
   this call only; `exclude` removes passes from the resolved set.
2. Print the result: which passes fired, their counts, and the next
   due time.

For `last`:

1. `dreaming({ cwd, args: '{"sub":"last"}' })` and print the `last_run`
   block.
...
- `memory_status` — pair with the `dreaming` status call when the user
  asks "is anything stale?" — that question means "should I run
  dreaming?"
- `dream_*` MCP tools — the lower-level durable job surface. The
  `dreaming` tool composes these but does not replace them.
