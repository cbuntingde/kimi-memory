...
| `skill`      | A v10 trigger-matching memory whose `metadata.trigger` shape (`{ commands?, paths?, keywords? }`) is matched against tool invocations by the internal `matchSkillTriggers` helper (`src/persist/skills.js`). There is no `match_skill_triggers` MCP tool. |
