...
The plugin exposes 52 MCP tools over the `kimi-memory` stdio server (37 always-on; 15 legacy tools gated behind `KIMI_MEMORY_LEGACY_SUBSYSTEMS=off`). This is the full catalog. Defaults: durable writes default to `scope: "project"`; durable reads default to `scope: "all"`.
...
- `memory_promote_to_global(cwd, memory_ids)` — move one or more project memories into the cross-project `_global` store immediately (no dry run; the `promote-to-global` CLI is the dry-run-by-default path). Returns `moved` / `skipped`.
...
- `dreaming(args)` — the user-facing wrapper around consolidate + dream + auto-GC. Takes a JSON-string `args` object; sees `sub` (`status` / `on` / `off` / `auto` / `run` / `last`), `scope`, `interval_spec`, `interval_ms`, `include`, `exclude`, `force`. The `operation` field in the response names the sub it ran (`dreaming_status`, `dreaming_set`, `dreaming_run`, `dreaming_last`).
