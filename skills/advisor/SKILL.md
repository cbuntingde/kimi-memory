...
- The auto-detect hook appends an `[advisor] matched: "<keyword>"` marker to the model's `hookSpecificOutput.additionalContext` on matching prompts; the marker tells the agent to load the `advisor` skill. It is not printed to the chat — the user-facing stdout is the single `[kimi-memory] …` recall line. So you may see the `[advisor]` marker in your input; treat it as a cue that the prompt matched a reflection keyword.
